...
interface Expense { id: string; name: string; value: number; }
...
export default function PdfGenerator({
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      function drawSection(title: string, items: Expense[], total: number, accentColor: [number, number, number]) {
        doc.setFillColor(24, 24, 27);
        const sectionH = 14 + items.length * 7 + 10;
      ...
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(113, 113, 122);
      doc.text('Salário', 22, y + 8);
      doc.text('Total Gastos', 22, y + 16);
      doc.text('Saldo Livre', 22, y + 24);

      doc.setTextColor(74, 222, 128);
      doc.text(`R$${salary.toLocaleString('pt-BR')}`, pageW - 18, y + 8, { align: 'right' });
      doc.setTextColor(248, 113, 113);
      doc.text(`R$${totalExpenses.toLocaleString('pt-BR')}`, pageW - 18, y + 16, { align: 'right' });
      doc.setTextColor(balance >= 0 ? 129 : 248, balance >= 0 ? 140 : 113, balance >= 0 ? 248 : 113);
      doc.text(`R$${balance.toLocaleString('pt-BR')}`, pageW - 18, y + 24, { align: 'right' });
